...
chatGPT_response <- POST(
  ...
  body = list(
    model = model,
    input = paste0(prompt_content,sample$content[q]) # 記得替換文件名和要替換的文本欄位
  )
)

res <- content(chatGPT_response, as="parsed", simplifyVector = FALSE)
result <- res[["output"]][[2]][["content"]][[1]][["text"]]
